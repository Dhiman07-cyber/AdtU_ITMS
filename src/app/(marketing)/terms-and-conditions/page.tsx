...
import { getLegalConfig, getSystemConfig } from '@/domains/admin';
import { ArrowLeft, Bookmark, LayoutDashboard, Zap } from 'lucide-react';
...
export default async function TermsAndConditionsPage() {
  let config: TermsConfig | null = null;
  let updatedAt: string | null = null;

  try {
    const termsConfigResult = await getLegalConfig('terms');
    let rawConfig = termsConfigResult.data;
    updatedAt = termsConfigResult.updatedAt;

    try {
      const systemConfigResult = await getSystemConfig();
      const appName = systemConfigResult.data?.appName || APP_NAME;
      if (rawConfig && typeof rawConfig === 'object') {
        let configStr = JSON.stringify(rawConfig);
        configStr = configStr.replace(/AdtU Bus Services/g, appName);
        rawConfig = JSON.parse(configStr);
      }
    } catch {
      // Graceful fallback to default appName
    }

    config = rawConfig as TermsConfig | null;
  } catch (error) {
    console.error('Failed to load terms on server:', error);
  }
