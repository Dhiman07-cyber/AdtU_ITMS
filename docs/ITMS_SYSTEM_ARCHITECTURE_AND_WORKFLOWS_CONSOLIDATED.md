...
*   **End Trip:** Driver calls `POST /api/driver/end-journey-v2`.
    *   *Supabase RPC:* executes `release_trip_lock` on `active_trips` table, resetting `driver_status` to `'idle'`.
...
## SECTION 14 — Complete Database Schema & Supabase Tables Reference
...
#### Table: `active_trips`
*   `trip_id` (TEXT PRIMARY KEY), `bus_id` (TEXT), `driver_id` (TEXT), `status` (TEXT), `started_at` (TIMESTAMPTZ)ng, required)
